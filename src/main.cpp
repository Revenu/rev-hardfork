...
unsigned int nStakeMinAge = 2 * 24 * nOneHour; //  days as zero time weight
unsigned int nStakeMaxAge = 365 * 24 * nOneHour; //  days as full weight
...
const string strMessageMagic = "Revenu Signed Message:\n";
...
int64_t GetProofOfWorkReward(unsigned int nBits, int64_t nFees, int nHeight)
{
	...
	if (nHeight < 200 && nHeight > 99)
		return 11362.5 * COIN;
    
	// Force block reward to zero when right shift is undefined.
    int halvings = nHeight / 430000;
    if (halvings >= 64)
        return nFees;
...
{
    int64_t nRewardCoinYear = 10 * CENT;
...
static const int64_t nTargetTimespan = 10 * 15 * 60;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Revenu
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // Revenu: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Revenu", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "REV - Genesis";
        CTransaction txNew;
        txNew.nTime = 1458835252;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1458835254;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1857542; // !fTestNet ? 1575379 : 46534;
