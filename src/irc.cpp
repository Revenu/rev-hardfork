...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #RevenuTEST2\r");
            Send(hSocket, "WHO #RevenuTEST2\r");
        } else {
            // randomly join #Revenu00-#Revenu05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Revenu%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Revenu%02d\r", channel_number).c_str());
        }
